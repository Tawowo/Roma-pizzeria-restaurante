  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [
        ...
        supabase.from('commandes').select('total').eq('statut', 'payee').gte('created_at', todayStr + 'T00:00:00').lte('created_at', todayStr + 'T23:59:59'),
        supabase.from('commandes').select('total').eq('statut', 'payee').gte('created_at', yesterdayStr + 'T00:00:00').lte('created_at', yesterdayStr + 'T23:59:59'),
        supabase.from('clients').select('*', { count: 'exact', head: true }),
        supabase.from('commandes').select('total').eq('statut', 'payee').gte('created_at', firstOfMonthStr + 'T00:00:00'),
        supabase.from('avis').select('*', { count: 'exact', head: true }).eq('statut', 'en_attente'),
        supabase.from('commandes').select('total, created_at').eq('statut', 'payee').gte('created_at', thirtyDaysAgoStr + 'T00:00:00'),
      ])
