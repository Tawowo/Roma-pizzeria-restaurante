...
export default function ComptePage() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setError(t('compte_err_suppression'))
    } finally {
      setLoading(false)
    }
  }
        ...
        <div style={{ width: '100%', maxWidth: screen === 'dashboard' ? 920 : 480 }}>
          ...
          {screen === 'home' && (
            <div style={{ textAlign: 'center' }}>
              ...
              <p style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 19, fontStyle: 'italic', color: 'var(--grigio)', marginBottom: 40 }}>{t('compte_sous_titre')}</p>
              <div style={{ display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap' }}>
                <button onClick={() => setScreen('login')} className="btn-primary" style={{ padding: '14px 32px', fontSize: 15 }}>
                  {t('compte_deja_compte')}
                </button>
              ...
              <div style={{ marginTop: 48, background: 'var(--verde-pale)', borderRadius: 4, padding: 28, textAlign: 'left' }}>
                <h3 style={{ fontFamily: 'Playfair Display, serif', fontSize: 20, color: 'var(--nero)', marginBottom: 16 }}>{t('compte_comment')}</h3>
                {[
                  { icon: '🍕', text: t('compte_gagner_commander') },
                  { icon: '📅', text: t('compte_gagner_reserver') },
                  { icon: '⭐', text: t('compte_gagner_avis') },
                  { icon: '🎂', text: t('compte_gagner_anniversaire') },
                ].map(item => (
          ...
          {screen === 'login' && (
            <>
              <div style={{ textAlign: 'center', marginBottom: 32 }}>
                <h1 style={{ fontFamily: 'Playfair Display, serif', fontSize: 32, color: 'var(--nero)', marginBottom: 8 }}>{t('compte_connexion')}</h1>
              ...
              <form onSubmit={handleLogin} style={{ background: 'white', border: '1px solid var(--grigio-l)', borderRadius: 4, padding: 'clamp(24px,5vw,40px)' }}>
                <label style={{ display: 'block', fontSize: 12, fontFamily: 'Jost', fontWeight: 500, color: 'var(--nero)', marginBottom: 8 }}>{t('compte_telephone')} *</label>
                ...
                <button type="button" onClick={() => { setScreen('home'); setError('') }} style={{ display: 'block', width: '100%', marginTop: 12, background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'Jost', fontSize: 13, color: 'var(--grigio)', textDecoration: 'underline' }}>← {t('retour')}</button>
              </form>
            </>
          )}
          ...
          {screen === 'create' && (
            <>
              <div style={{ textAlign: 'center', marginBottom: 32 }}>
                <h1 style={{ fontFamily: 'Playfair Display, serif', fontSize: 32, color: 'var(--nero)', marginBottom: 8 }}>{t('compte_creer')}</h1>
                <p style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 18, fontStyle: 'italic', color: 'var(--grigio)' }}>{t('compte_rejoindre')}</p>
              </div>
              <form onSubmit={handleCreate} style={{ background: 'white', border: '1px solid var(--grigio-l)', borderRadius: 4, padding: 'clamp(24px,5vw,40px)' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
                  <div>
                    <label style={{ display: 'block', fontSize: 12, fontFamily: 'Jost', fontWeight: 500, color: 'var(--nero)', marginBottom: 8 }}>{t('compte_prenom')} *</label>
                    <input type="text" className="form-input" placeholder={t('compte_prenom')} value={prenom} onChange={e => setPrenom(e.target.value)} required />
                  </div>
                  <div>
                    <label style={{ display: 'block', fontSize: 12, fontFamily: 'Jost', fontWeight: 500, color: 'var(--nero)', marginBottom: 8 }}>{t('compte_nom_label')} *</label>
                    <input type="text" className="form-input" placeholder={t('compte_nom_label')} value={nom} onChange={e => setNom(e.target.value)} required />
                  </div>
                </div>
                ...
                <div style={{ marginBottom: 20 }}>
                  <label style={{ display: 'block', fontSize: 12, fontFamily: 'Jost', fontWeight: 500, color: 'var(--nero)', marginBottom: 8 }}>{t('compte_email')}</label>
          ...
          {screen === 'dashboard' && client && (
            <>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 32, flexWrap: 'wrap' }}>
                ...
                <div>
                  <div style={{ fontFamily: 'Jost', fontSize: 11, letterSpacing: 2, textTransform: 'uppercase', color: 'var(--verde)', marginBottom: 4 }}>{t('compte_bienvenue')}</div>
                ...
                <div style={{ marginLeft: 'auto', background: niveau.bg, color: niveau.color, padding: '8px 16px', borderRadius: 20, fontSize: 13, fontFamily: 'Jost', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 }}>
                  {niveau.icon} {t('compte_membre')} {niveau.label}
                </div>
              </div>
              ...
              <div style={{ background: 'var(--hero-bg)', borderRadius: 4, padding: '28px 32px', marginBottom: 32, position: 'relative', overflow: 'hidden', border: '1px solid rgba(27,94,32,0.3)' }}>
                ...
                <div style={{ fontFamily: 'Jost', fontSize: 10, letterSpacing: 3, textTransform: 'uppercase', color: 'rgba(255,255,255,0.35)', marginBottom: 8 }}>{t('compte_mes_points')}</div>
                <div style={{ fontFamily: 'Playfair Display, serif', fontSize: 52, fontWeight: 600, color: 'var(--rosso)', lineHeight: 1, marginBottom: 16 }}>
                  ...
                  <span style={{ fontFamily: 'Jost', fontSize: 15, fontWeight: 300, color: 'rgba(255,255,255,0.35)', marginLeft: 8 }}>{t('compte_pts')}</span>
                </div>
                ...
                <p style={{ fontFamily: 'Jost', fontSize: 12, color: 'rgba(255,255,255,0.4)' }}>
                  {client.points_fidelite} / {maxPoints} {t('compte_pts')} — {t('compte_prochaine_recompense')} : {RECOMPENSES.find(r => r.points > client.points_fidelite)?.label ?? t('compte_toutes_debloquees')}
                </p>
              </div>

              {/* Catalogue récompenses */}
              <div style={{ marginBottom: 32 }}>
                <h2 style={{ fontFamily: 'Playfair Display, serif', fontSize: 24, color: 'var(--nero)', marginBottom: 8 }}>🎁 {t('compte_mes_recompenses')}</h2>
                <p style={{ fontSize: 13, color: 'var(--grigio)', fontFamily: 'Jost', marginBottom: 20 }}>{t('compte_recompenses_desc')}</p>
                      ...
                      <div key={r.label} style={{ background: 'white', border: `1px solid ${unlocked ? 'var(--verde)' : 'var(--grigio-l)'}`, borderRadius: 3, padding: 16, opacity: unlocked ? 1 : 0.75 }}>
                        ...
                        {unlocked ? (
                          ...
                          <div style={{ fontSize: 11, color: 'var(--grigio)', fontFamily: 'Jost' }}>
                            {t('compte_encore')} {r.points - client.points_fidelite} {t('compte_pts_manquants')}
                          </div>
                        )}
                      </div>
                    )
              ...
              {bonCode && (
                <div style={{ background: 'var(--verde-pale)', border: '2px solid var(--verde)', borderRadius: 4, padding: 24, marginBottom: 32, textAlign: 'center' }}>
                  ...
                  <h3 style={{ fontFamily: 'Playfair Display, serif', fontSize: 22, color: 'var(--verde)', marginBottom: 8 }}>{t('compte_bon_titre')}</h3>
                  ...
                  <p style={{ fontSize: 13, color: 'var(--verde-m)', fontFamily: 'Jost' }}>{t('compte_bon_desc')} ✓</p>
                  <button onClick={() => setBonCode(null)} style={{ marginTop: 12, background: 'none', border: 'none', color: 'var(--grigio)', cursor: 'pointer', fontFamily: 'Jost', fontSize: 12, textDecoration: 'underline' }}>{t('fermer')}</button>
                </div>
              )}

              {/* Comment gagner des points */}
              <div style={{ background: 'var(--verde-pale)', borderRadius: 4, padding: 24, marginBottom: 32 }}>
                <h3 style={{ fontFamily: 'Playfair Display, serif', fontSize: 20, color: 'var(--nero)', marginBottom: 16 }}>{t('compte_comment_gagner')}</h3>
                {[
                  { icon: '🍕', text: t('compte_gagner_commander') },
                  { icon: '📅', text: t('compte_gagner_reserver') },
                  { icon: '⭐', text: t('compte_gagner_avis') },
              ...
              {/* Réservations */}
              <div style={{ marginBottom: 32 }}>
                <h2 style={{ fontFamily: 'Playfair Display, serif', fontSize: 24, color: 'var(--nero)', marginBottom: 16 }}>{t('compte_mes_reservations')}</h2>
                {reservations.length === 0 ? (
                  <div style={{ background: 'white', border: '1px solid var(--grigio-l)', borderRadius: 4, padding: 24, textAlign: 'center' }}>
                    <p style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 17, fontStyle: 'italic', color: 'var(--grigio)' }}>{t('compte_aucune_reservation')}</p>
                    <Link href="/#reserver" className="btn-primary" style={{ display: 'inline-block', textDecoration: 'none', marginTop: 12, fontSize: 13, padding: '10px 24px' }}>{t('reserver_titre')}</Link>
                  </div>
                ) : (
                    ...
                    {reservations.map(r => (
                      <div key={r.id} style={{ background: 'white', border: '1px solid var(--grigio-l)', borderRadius: 4, padding: '14px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
                        <div>
                          ...
                          <p style={{ fontFamily: 'Jost', fontSize: 12, color: 'var(--grigio)' }}>{r.nombre_couverts} {t('compte_personnes')}{r.zone ? ` · ${r.zone}` : ''}</p>
                        </div>
                        <span style={{
                          ...
                          {r.statut === 'en_attente' ? t('compte_statut_attente') : r.statut === 'confirmee' ? t('compte_statut_confirmee') : r.statut === 'annulee' ? t('compte_statut_annulee') : t('compte_statut_honoree')}
                        </span>
                      </div>
                    ))}
                  </div>
              ...
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 32 }}>
                <Link href="/#reserver" className="btn-primary" style={{ fontSize: 13, textDecoration: 'none' }}>📅 {t('reserver_titre')}</Link>
                <Link href="/#commander" className="btn-secondary" style={{ fontSize: 13, textDecoration: 'none' }}>🍕 {t('emporter_titre')}</Link>
                  ...
                  <div style={{ background: 'var(--rosso-pale)', border: '1px solid var(--rosso-l)', borderRadius: 4, padding: 20 }}>
                    ...
                    <div style={{ display: 'flex', gap: 12 }}>
                      <button onClick={handleDelete} disabled={loading} style={{ background: 'var(--rosso)', color: 'white', border: 'none', padding: '8px 20px', borderRadius: 3, fontSize: 13, cursor: 'pointer', fontFamily: 'Jost', opacity: loading ? 0.7 : 1 }}>
                        {loading ? '...' : t('compte_confirmer')}
                      </button>
                      <button onClick={() => setDeleteConfirm(false)} style={{ background: 'none', border: '1px solid var(--grigio-l)', color: 'var(--grigio)', padding: '8px 16px', borderRadius: 3, fontSize: 13, cursor: 'pointer', fontFamily: 'Jost' }}>{t('annuler')}</button>
                    </div>
                  </div>
                )}
              </div>
